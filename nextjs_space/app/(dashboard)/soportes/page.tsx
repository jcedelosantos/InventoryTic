...
export default function SoportesPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    fetch(`/api/supports?${params}`).then(r => r.json()).then(d => { setItems(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false));
  }, [selectedClientId]);
  useEffect(() => { fetchData(); }, [fetchData]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const body = { ...form, costoMensual: parseFloat(form?.costoMensual) || 0, costoAnual: parseFloat(form?.costoAnual) || 0, clientId: selectedClientId || undefined };
